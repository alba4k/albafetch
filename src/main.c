...
int main(int argc, char **argv) {
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        if(strcmp(argv[i], "-v") == 0 || strcmp(argv[i], "--version") == 0) {
            ...
            safe_strncpy(config_file, argv[i + 1], sizeof(config_file));
            continue;
    ...
    if(_align) {
        ...
        for(struct Module *current = modules->next; current; current = current->next) {
            current_len = real_strlen(current->label);

            if(current_len > (size_t)asking_align)
                asking_align = (int)current_len;
        }

        asking_align += (int)real_strlen(config.dash);
    ...
    for(struct Module *current = modules->next; current; current = current->next) {
        if(strcmp(current->id, "separator") == 0) { // separators are handled differently
            ...
            const size_t len = real_strlen(printed) - real_strlen(config.separator_prefix) - (print_logo ? real_strlen(config.logo[2]) + config.spacing : 0);
